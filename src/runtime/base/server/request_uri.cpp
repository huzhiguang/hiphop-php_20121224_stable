...
static bool MatchAnyPattern(const std::string &path, const std::vector<std::string> &patterns) {
	String spath(path.c_str(), path.size(), AttachLiteral);
	for (unsigned int i = 0; i < patterns.size(); i++) {
		Variant ret = preg_match(String(patterns[i].c_str(), patterns[i].size(),AttachLiteral), spath);
...
RequestURI::RequestURI(const VirtualHost *vhost, Transport *transport,
  ...
  if (!process(vhost, transport, sourceRoot, pathTranslation,
    ...
    // check reverse proxy config: add by yfliulei@360buy.com 2012-11-1
    //
    // proxy any URLs that not specified in ServeURLs
    if (!RuntimeOption::ProxyOrigin.empty() &&
          ((RuntimeOption::UseServeURLs &&
            RuntimeOption::ServeURLs.find(path().data()) ==
            RuntimeOption::ServeURLs.end()) ||
           (RuntimeOption::UseProxyURLs &&
            (RuntimeOption::ProxyURLs.find(path().data()) !=
             RuntimeOption::ProxyURLs.end() ||
             MatchAnyPattern(path().data(), RuntimeOption::ProxyPatterns) ||
