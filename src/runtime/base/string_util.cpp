...
String StringUtil::HtmlEncode_GB2312(CStrRef input, QuoteStyle quoteStyle, bool nbsp) {
  if (input.empty()) return input;
  int len = input.size();
  bool utf8 = false;
  char *ret = string_html_encode_GB2312(input, len, quoteStyle != NoQuotes, quoteStyle == BothQuotes, utf8, nbsp);
