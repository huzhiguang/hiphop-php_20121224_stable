...
#define CHECK_LEN(pos, chars_need, len) ((len - (pos)) >= (chars_need))
#define ENCODE_FAILURE(p, len) do {\
	free(p);\
	len=0 ;\
	return NULL ;\
} while(0)
...
char *string_html_encode_GB2312(const char *input, int &len, bool encode_double_quote, bool encode_single_quote, bool utf8, bool nbsp) {
  ASSERT(input);
  /**
   * Though seems to be wasting memory a lot, we have to realize most of the
   * time this function is called with small strings, or fragments of HTMLs.
  ...
  for (const char *p = input, *end = input + len; p < end; p++, pos++) {
    char c = *p;
    cursor = (char*)p ;
    if ((unsigned char)c >= 0xA1 && (unsigned char)c <= 0xFE) {				// is GB2312
    	unsigned char next;
    	if (!CHECK_LEN(pos, 2, len))
    		ENCODE_FAILURE(ret, len);
		...
		pos += 1;
		p++ ;
    } else if (gb2312_lead((unsigned char)c)) {
    	switch (c) {
		case '"':
		  if (encode_double_quote) {
			*q++ = '&'; *q++ = 'q'; *q++ = 'u'; *q++ = 'o'; *q++ = 't'; *q++ = ';';
