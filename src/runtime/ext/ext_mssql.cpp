...
DBPROCESS *MSSQL::create_new_conn(){
	RETCODE retCode;
	DBPROCESS * m_conn_new=NULL;
	retCode = dbinit();
	if (retCode == FAIL) {
		raise_error("dbinit() failed\n");
	}
	if(loginRec){
		 dbloginfree(loginRec);	
		 loginRec=NULL;
	...
	if(!loginRec)
	{
		raise_error( "unable to allocate login structure\n");
	}
	dbmsghandle((MHANDLEFUNC)f_msg_handler);
//	dbprocmsghandle(loginRec, (MHANDLEFUNC) f_msg_handler);
	DBSETLUSER(loginRec, m_username.c_str());
	DBSETLPWD(loginRec, m_password.c_str());
	if ((m_conn_new = dbopen(loginRec, m_server.c_str())) == NULL)									 
	{
...
//		raise_error("runtime/ext_mssql: failed executing [%s]", query.data());
//	}
	...
	{
		raise_error("Waring: [%p]",mssql_glob_proc);
		raise_error("runtime/ext_mssql: failed executing [%s]", query.data());
		return false;
	}
	...
	while(dbresults(conn) != NO_MORE_RESULTS){
		...
		while((row_code = dbnextrow(conn)) != NO_MORE_ROWS){
			...
			for(int c = 0 ; c < num_fields ; c++){
				...
				if(NULL == pdata){
					raise_error("calloc memory address failed!");
					return rconn;
				}
...
Variant f_mssql_connect(CStrRef server /* = null_string */, CStrRef username /* = null_string */, CStrRef password /* = null_string */, bool new_link /* = false */) {
	if(RuntimeOption::MsSQLISPERSISTENT){
				return f_mssql_pconnect(server,username,password);
	}else{
		//connect
		Object ret;
...
//		if(dbdead(m_msSQL) == TRUE ){
		...
		if(dbdeadflag){
			...
			msSQL->m_conn=msSQL->create_new_conn();
