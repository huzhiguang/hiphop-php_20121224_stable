...
#define DEFAULT_COMPRESSION_TYPE COMPRESSION_TYPE_FASTLZ
...
#define MEMC_COMPRESS_THRESHOLD 2000
...
void c_Memcached::toPayload(CVarRef value, vector<char> &payload,
  ...
  if (m_impl->compression && encoded.length() >= MEMC_COMPRESS_THRESHOLD) {
    ...
    if (DEFAULT_COMPRESSION_TYPE == COMPRESSION_TYPE_FASTLZ) {
	  //**********************************************************************************
	  // compressing data with fastlz. yfliulei@360buy.com 2012-10-17
	  //
	  payloadCompLength = encoded.length() ;
	  payload.resize(payloadCompLength+sizeof(uint32_t)) ;
	  char* payload_comp = payload.data() ;

	  memcpy(payload_comp, &payloadCompLength, sizeof(uint32_t));
	  payload_comp += sizeof(uint32_t);
	  unsigned long payload_comp_len = fastlz_compress(encoded.data(), payloadCompLength, payload_comp) ;
	  flags |= MEMC_VAL_COMPRESSED;
	  flags |= MEMC_VAL_COMPRESSION_FASTLZ;
	  if (payloadCompLength > payload_comp_len) {
		  size_t payload_len ;
		  payload_len = payload_comp_len + sizeof(uint32_t);
		  payload_comp[payload_len] = 0 ;
		  payload.resize(payload_len);
		  return ;
	  }
	  //**********************************************************************************
    } else if (DEFAULT_COMPRESSION_TYPE == COMPRESSION_TYPE_ZLIB) {
	  payloadCompLength = compressBound(encoded.length());
	  payload.resize(payloadCompLength);
...
bool c_Memcached::toObject(Variant& value, const memcached_result_st &result) {
  ...
  if (flags & MEMC_VAL_COMPRESSED) {
    ...
    if ( flags & MEMC_VAL_COMPRESSION_FASTLZ) {
		// *******************************************************************
		// uncompress with fastlz 2012-10-16 yfliulei@360buy.com
		//
		uint32_t len;
		...
		unsigned long length = fastlz_decompress(payload, payloadLength, (char*)buffer.data(), len);
		...
		for (int factor = 1; !done && factor <= 16; ++factor) {
		  ...
		  if (uncompress((Bytef*)buffer.data(), &bufferSize, (const Bytef*)payload, (uLong)payloadLength) == Z_OK) {
			done = true;
		  }
		}
	}
