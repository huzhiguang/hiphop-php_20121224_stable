...
HPHP=$HPHP_HOME/build/src/hphp/hphp
[ ! -x "$HPHP" ] && check_err 1 "$HPHP is not executable"

HHVM=$HPHP_HOME/build/src/hhvm/hhvm
...
if [ "$1" = "gen" -o "$1" = "all" -o "$1" = "all_s" ]; then
...
if [ "$1" = "hhvm" -o "$1" = "all" ]; then
  ...
  RUNTIME_BUILD=$HPHP_HOME/build/src/CMakeFiles/hphp_runtime_static.dir
  ...
  EXTHHVM_BUILD=$HPHP_HOME/build/src/runtime/ext_hhvm/CMakeFiles/ext_hhvm_static.dir
  make_hhvm $EXTHHVM_BUILD/ext_hhvm_noinline.cpp.o ext_hhvm_noinline.ext_hhvm.cpp
fi

if [ "$1" = "hhvm_s" -o "$1" = "all_s" ]; then
  cd $HPHP_HOME/src/runtime/ext
  #SOURCES=`find . -name '*.cpp' | grep -v ext_hhvm | grep -v sep | cut -c 3- | cut -d . -f 1`
  RUNTIME_BUILD=$HPHP_HOME/build/src/CMakeFiles/hphp_runtime_static.dir
  cd $HPHP_HOME/src/runtime/ext_hhvm
  ...
  #fi
  if [ "$2" != "" ]; then
    make_hhvm $RUNTIME_BUILD/runtime/ext/ext_$2.cpp.o ../ext/ext_$2.ext_hhvm.cpp
  	make_hhvm $RUNTIME_BUILD/runtime/base/builtin_functions.cpp.o ../base/builtin_functions.ext_hhvm.cpp
  	EXTHHVM_BUILD=$HPHP_HOME/build/src/runtime/ext_hhvm/CMakeFiles/ext_hhvm_static.dir
  	make_hhvm $EXTHHVM_BUILD/ext_hhvm_noinline.cpp.o ext_hhvm_noinline.ext_hhvm.cpp
  fi
...
if [ "$1" = "infotabs" -o "$1" = "all" -o "$1" = "all_s" ]; then
  cd $HPHP_HOME/src/runtime/ext_hhvm
...
if [ "$1" = "systemlib" -o "$1" = "all" -o "$1" = "all_s" ]; then
  cd $HPHP_HOME
...
if [ "$1" = "injection" -o "$1" = "all" -o "$1" = "all_s" ]; then
  cd $HPHP_HOME
...
if [ "$1" = "license" -o "$1" = "all" -o "$1" = "all_s" ]; then
  cd $HPHP_HOME
